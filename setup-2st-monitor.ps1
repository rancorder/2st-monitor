...
# ============================================
# 実行方法:
# irm https://your-gist-url/setup-2st-monitor.ps1 | iex
# ============================================
...
function Get-MonitorScript {
    ...
    # ここに2st-monitor.jsのURL（GitHub Gist等）を指定
    $scriptUrl = "https://raw.githubusercontent.com/YOUR_USERNAME/YOUR_REPO/main/2st-monitor.js"
    $scriptPath = Join-Path $ProjectPath "2st-monitor.js"
    
    # スクリプトURLが設定されていない場合のプレースホルダー
    Write-ColorOutput "注意: 2st-monitor.js は手動で配置してください" "Warning"
    Write-ColorOutput "  配置先: $scriptPath" "Info"
    ...
    #     return $true
    # } catch {
    #     Write-ColorOutput "2st-monitor.jsのダウンロードに失敗しました" "Error"
    #     return $false
    # }
...
function Main {
    ...
    Write-Host "  1. 2st-monitor.js をプロジェクトフォルダに配置" -ForegroundColor White
    Write-Host "     (まだ配置されていない場合)" -ForegroundColor Gray
    Write-Host ""
